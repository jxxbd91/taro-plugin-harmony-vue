...
const getPolicy = () => {
  ...
  if (tt) {
    try {
      ...
      !!("production" !== "production") && warn(`Error creating trusted types policy: ${e}`);
    }
  }
  return policy
}
...
const getTemplateContainer = () => getDoc() && /* @__PURE__ */ getDoc().createElement("template");
const nodeOps = {
  ...
  createElement: (tag, namespace, is, props) => {
    const el = namespace === "svg" ? getDoc().createElementNS(svgNS, tag) : namespace === "mathml" ? getDoc().createElementNS(mathmlNS, tag) : is ? getDoc().createElement(tag, { is }) : getDoc().createElement(tag);
    if (tag === "select" && props && props.multiple != null) {
      el.setAttribute("multiple", props.multiple);
    }
    return el;
  },
  createText: (text) => getDoc().createTextNode(text),
  createComment: (text) => getDoc().createComment(text),
  setText: (node, text) => {
    node.nodeValue = text;
  },
  ...
  querySelector: (selector) => getDoc().querySelector(selector),
  setScopeId(el, id) {
    el.setAttribute(id, "");
  },
  // __UNSAFE__
  ...
  insertStaticContent(content, parent, anchor, namespace, start, end) {
    ...
    if (start && (start === end || start.nextSibling)) {
      ...
      getTemplateContainer().innerHTML = getUnsafeToTrustedHTML()(
        namespace === "svg" ? `<svg>${content}</svg>` : namespace === "mathml" ? `<math>${content}</math>` : content
      );
      const template = getTemplateContainer().content;
...
function forceReflow() {
  return get('document').body.offsetHeight;
}
...
function useCssVars(getter) {
  ...
  if (!instance) {
    !!("production" !== "production") && warn(`useCssVars is called without current active component instance.`);
    return;
  }
  const updateTeleports = instance.ut = (vars = getter(instance.proxy)) => {
    Array.from(
      get('document').querySelectorAll(`[data-v-owner="${instance.uid}"]`)
...
function patchDOMProp(el, key, value, parentComponent) {
  if (key === "innerHTML" || key === "textContent") {
    if (value != null) {
      el[key] = key === "innerHTML" ? getUnsafeToTrustedHTML()(value) : value;
    }
    return;
  }
...
class VueElement extends BaseClass {
  ...
  _resolveDef() {
    ...
    const resolve = (def, isAsync = false) => {
      ...
      } else if (!!("production" !== "production") && styles) {
        warn(
  ...
  _mount(def) {
    if ((!!("production" !== "production") || __VUE_PROD_DEVTOOLS__) && !def.name) {
      def.name = "VueElement";
    }
  ...
  _applyStyles(styles, owner) {
    ...
    for (let i = styles.length - 1; i >= 0; i--) {
      const s = get('document').createElement("style");
      ...
      if (content) {
        for (const n of content) {
          if (scopeId && n.nodeType === 1) {
            const id = scopeId + "-s";
            const walker = get('document').createTreeWalker(n, 1);
            n.setAttribute(id, "");
            let child;
...
function useShadowRoot() {
  const el = !!("production" !== "production") ? useHost("useShadowRoot") : useHost();
...
function useCssModule(name = "$style") {
  {
    ...
    if (!modules) {
      !!("production" !== "production") && warn(`Current instance does not have CSS modules injected.`);
      return EMPTY_OBJ;
    }
    const mod = modules[name];
    if (!mod) {
      !!("production" !== "production") && warn(`Current instance does not have CSS module named "${name}".`);
      return EMPTY_OBJ;
    }
    return mod;
  }
    ...
    return () => {
      ...
      if (children) {
        for (let i = 0; i < children.length; i++) {
          ...
          if (child.el && child.el instanceof get('TaroElement')) {
...
const vModelText = {
  ...
  beforeUpdate(el, { value, oldValue, modifiers: { lazy, trim, number } }, vnode) {
    ...
    if (get('document').activeElement === el && el.type !== "range") {
...
const createApp = (...args) => {
  ...
  app.mount = (containerOrSelector) => {
    ...
    if (container instanceof get('TaroElement')) {
...
function resolveRootNamespace(container) {
  if (container instanceof getSvgElement()) {
    return "svg";
  }
...
function normalizeContainer(container) {
  if (isString(container)) {
    const res = get('document').querySelector(container);
