...
import { parseVueRequest as originParseVueRequest } from '@vitejs/plugin-vue'

import { appendVirtualModulePrefix, stripVirtualModulePrefix } from '@tarojs/vite-runner/dist//utils'
import {
  checkPublicFile,
  fileToUrl,
  // generatedAssets,
  ...
  renderAssetUrlInJS,
} from '@tarojs/vite-runner/dist/harmony/asset'
import { compileCSS } from '@tarojs/vite-runner/dist/harmony/postcss'
import {
  commonjsProxyRE, CSS_LANGS_RE, cssModuleRE,
  htmlProxyRE, inlineCSSRE, inlineRE, loadParseImportRE,
  SPECIAL_QUERY_RE, usedRE
} from '@tarojs/vite-runner/dist/harmony/postcss/constants'
import { finalizeCss, stripBomTag } from '@tarojs/vite-runner/dist/harmony/postcss/utils'
...
import type { CssUrlReplacer } from '@tarojs/vite-runner/dist/harmony/postcss/types'
...
interface VueQuery {
  vue?: boolean;
  src?: string;
  type?: 'script' | 'template' | 'style' | 'custom' | 'harmony-style';
  index?: number;
  lang?: string;
  raw?: boolean;
  url?: boolean;
...
const parseVueRequest = (id: string) => originParseVueRequest(id) as {
  filename: string;
  query: VueQuery;
};;
...
function rmJsxCode(raw: string, id: string) {
  const result = transformSync(raw, {
    ...
    plugins: [
      function rmJsx(babel: typeof BabelCore): BabelCore.PluginObj<BabelCore.PluginPass> {
        const t = babel.types
        return {
          name: "taro-rm-jsx-plugin",
          visitor: {
            ExportDefaultDeclaration(ast) {
              const declaration = ast.node.declaration
              if (t.isFunctionDeclaration(declaration) && declaration.id?.name === 'Index') {
                ast.remove()
              }
            }
          }
...
function setCssToInlineStyle(raw: string, id: string) {
  const result = transformSync(raw, {
    filename: id,
    plugins: [
      function cssToInlineStyle(babel: typeof BabelCore): BabelCore.PluginObj<BabelCore.PluginPass> {
        const t = babel.types
        return {
          name: "taro-css-to-style-plugin",
          visitor: {
            CallExpression(ast) {
              const {callee, arguments: args} = ast.node
              if (t.isIdentifier(callee) && callee.name === 'createElementVNode' && args[1]) {
                const opt = args[1]
                if (t.isObjectExpression(opt)) {
                  const classProperty = opt.properties.find(item => t.isObjectProperty(item) && t.isIdentifier(item.key) && item.key.name === 'class') 
                  if (t.isObjectProperty(classProperty) && t.isStringLiteral(classProperty.value)) {
                    opt.properties.push(
                      t.objectProperty(
                        t.identifier('style'),
                        t.callExpression(
                          t.identifier('calcStaticStyle'),
                          [
                            t.callExpression(
                              t.identifier('__inner_style__'),
                              []
                            ),
                            t.stringLiteral(classProperty.value.value)
                          ]
                        )
                      )
                    )
  ...
  return {
    ...
    resolveId (source, importer) {
      if (CSS_LANGS_RE.test(source)) {
        ...
        // modify
        if (suffix?.includes('type=style')&&suffix?.includes('vue')) {
          return null;
        }
        if (virtualId) {
    ...
    load (id) {
      ...
      if (isStyleRequest(id) && !(query.type === 'style' && query.vue)) {
    ...
    async transform(raw, id) {
      ...
      if (query.type === 'style') {
        cssCache.set(filename, raw)
        return;
      // @ts-ignore
      } else if (query.type === 'harmony-style') {
        const content = cssCache.get(filename);
        if (content) {
          let { code: raw_code } = parseJSXStyle(`export default function Index () {return <View className="index"></View>}`, [content], {
              platformString: 'Harmony'
          });

          raw_code = raw_code.replace('@tarojs/runtime', '../../npm/@tarojs/runtime');
          raw_code = rmJsxCode(raw_code, id)
          cssCache.set(id, raw_code)
          styles.set(filename, raw_code);
        }
      }
  ...
  return {
    ...
    async transform(raw, id) {
      ...
      if (
        ...
        query.type !== "harmony-style"
      ) {
        if (raw.includes("?vue&type=style")) {
          const code = `
                  ${raw};
                  import '${id}?harmony&type=harmony-style'
              `;
          return {
            code,
            map: null,
    ...
    async renderChunk(code, chunk, opts) {
      ...
      if (opts.format === 'es' || opts.format === 'cjs') {
        ...
        const s = new MagicString(chunkCSS + '\n' + setCssToInlineStyle(code, chunk.fileName));
        return {
          code: s.toString(),
          map: s.generateMap({ hires: true }),
        };
